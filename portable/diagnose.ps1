...
[void]$r.AppendLine('===== SpiritVale 紫星販賣保護 診斷報告 =====')
...
if (-not $game) {
    ...
    $plugin = Join-Path $game 'BepInEx\plugins\SpiritValeSellFavorite\SpiritValeSellFavorite.dll'
    if (Test-Path $plugin) {
        $p = Get-Item $plugin
    ...
    $others = Get-ChildItem (Join-Path $game 'BepInEx\plugins') -Recurse -Filter '*.dll' | Where-Object Name -ne 'SpiritValeSellFavorite.dll'
    if ($others) {
        [void]$r.AppendLine('其他外掛: ' + (($others | ForEach-Object { $_.Name }) -join ', '))
    }

    $cfg = Join-Path $game 'BepInEx\config\local.spiritvale.sellfavorite.cfg'
    if (Test-Path $cfg) {
        [void]$r.AppendLine('')
    ...
    $marks = Join-Path $game 'BepInEx\config\local.spiritvale.sellfavorite.marks.txt'
    if (Test-Path $marks) {
        [void]$r.AppendLine('紫星數量: ' + @(Get-Content $marks | Where-Object { $_.Trim() }).Count)
    }
    ...
    if (Test-Path $log) {
        ...
        Get-Content $log | Where-Object { $_ -match '紫星|SellFavorite|Error|Exception|Fatal|Warning' } |
...
$out = Join-Path ([Environment]::GetFolderPath('Desktop')) '紫星販賣保護_診斷報告.txt'
